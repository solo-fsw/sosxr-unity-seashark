...
namespace SOSXR.Extensions
{
    public static class ExtensionMethods
    {
        ...
        public static void AddPersistentCallersToUnityEvent(this UnityEvent unityEvent, IEnumerable<UnityAction> persistentCallers)
        {
            foreach (var persistentCaller in persistentCallers)
            {
                for (var i = 0; i < unityEvent.GetPersistentEventCount(); i++)
                {
                    if (unityEvent.GetPersistentTarget(i) == (Object) persistentCaller.Target && unityEvent.GetPersistentMethodName(i) == persistentCaller.Method.Name)
                    {
                        Debug.Log("AddPersistentCallersToUnityEvent" + "Already exists in the UnityEvent you're trying to add it to");
                ...
                Debug.Log("AddPersistentCallersToUnityEvent" + "Not yet in list, will add");
                UnityEventTools.AddPersistentListener(unityEvent, persistentCaller);
                #else
                Debug.LogError("AddPersistentCallersToUnityEvent" + "Cannot call this outside of the editor");
                #endif
            }
        }
