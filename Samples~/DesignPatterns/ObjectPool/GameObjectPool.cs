...
namespace SOSXR.SeaShark
{
    ...
    public class GameObjectPool : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (m_prefab == null)
            {
                Debug.LogError($"{nameof(GameObjectPool)} on '{name}' requires a prefab reference.", this);
                return;
            }
        ...
        public GameObject Get(Vector3 position, Quaternion rotation)
        {
            if (m_prefab == null)
            {
                throw new InvalidOperationException($"{nameof(GameObjectPool)} on '{name}' cannot provide objects because no prefab is assigned.");
            }
